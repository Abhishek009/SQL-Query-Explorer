...
.menu{position:absolute;top:calc(100% + 4px);right:0;z-index:6;min-width:100px;flex-direction:column;gap:2px;padding:4px;background:var(--vscode-menu-background,var(--vscode-editor-background,#1f1f1f));color:var(--vscode-menu-foreground,var(--vscode-foreground));border:1px solid var(--vscode-menu-border,var(--vscode-panel-border,rgba(128,128,128,.45)));border-radius:4px;box-shadow:0 4px 12px rgba(0,0,0,.35)}
.menu:not([hidden]){display:flex}
.menu-item{width:100%;text-align:left;padding:5px 10px;background:transparent;color:inherit;border:0;border-radius:3px}
.menu-item:hover{background:var(--vscode-menu-selectionBackground,var(--vscode-list-hoverBackground,rgba(128,128,128,.2)));color:var(--vscode-menu-selectionForeground,inherit)}
...
th.sorted{text-decoration:underline;text-underline-offset:3px}
...
#filter{width:150px}</style></head><body>${toolbar}${table}<script nonce="${nonce}">const vscode=acquireVsCodeApi();const box=document.getElementById('limit');const send=()=>{const v=Number(box.value);if(Number.isFinite(v)&&v>0)vscode.postMessage({type:'limit',value:v});};box.addEventListener('change',send);box.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();send();}});const exportToggle=document.getElementById('export-toggle');const exportMenu=document.getElementById('export-menu');
exportToggle.addEventListener('click',e=>{e.stopPropagation();exportMenu.hidden=!exportMenu.hidden;});
exportMenu.querySelectorAll('.menu-item').forEach(item=>item.addEventListener('click',()=>{exportMenu.hidden=true;vscode.postMessage({type:'download',format:item.dataset.format});}));

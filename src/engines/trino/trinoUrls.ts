 ...
 * Values already typed into those fields win over the ones in the URL. Trino
 * only: Postgres has no equivalent connection-string paste.
 */
export function expandPastedUrl(message: ConnectionMessage): ConnectionMessage {
    if (message.engine !== 'trino') { return message; }
